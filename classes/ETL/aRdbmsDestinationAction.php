...
abstract class aRdbmsDestinationAction extends aAction
{
    ...
    /**
     * Log any warnings returned by the database. It is assumed that the warnings provided
     * has been returned by the SHOW WARNINGS command and each row contains three columns
     * (level, code, message). The level can be (Error, Warning, Note) note the first
     * letter is capitalized. If the warning was generated by executing an SHOW WARNING
     * query directly each row will already be an array with 3 elements. If the warning
     * was generated as part of a LOAD DATA INFILE command each row will be a string
     * returned by the command and will need to be split.
     *
     * Error is typically returned if running SHOW WARNINGS after a failed SQL statement
     * and Note is typically returned after a command that did not do anything but
     * contained an IF EXISTS clause.
     *
     * @param array $warnings The results from the SHOW WARNINGS command.
     * @param string $tableName The name of the table that the warnings are related to.
     *
     * @return int The number of warnings logged.
     */

    protected function logSqlWarnings(array $warnings, $tableName)
    {
        if ( $this->options->hide_sql_warnings ) {
            return 0;
        }

        $msg = sprintf("SQL warnings on table '%s' generated by action %s", $tableName, $this->name);
        ...
        foreach ( $warnings as $row ) {
            if ( null !== $this->options->hide_sql_warning_codes ) {
                ...
                if ( in_array($code, $this->options->hide_sql_warning_codes) ) {
                    continue;
                }
            }
